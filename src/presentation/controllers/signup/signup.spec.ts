...
const makeFakeRequest = (deleteParams?: [string]): HttpRequest => {
  const httpRequest = {
    body: {
      name: 'valid_name',
      email: 'valid_email@mail.com',
  ...
  deleteParams?.map(param => delete httpRequest.body[param])
  return httpRequest
}
interface SutTypes {
...
describe('SignUp Controller', () => {
  test('Should return 400 if no name is provider', async () => {
    const { sut } = makeSut()
    const httpRequest = makeFakeRequest(['name'])
  ...
  test('Should return 400 if no email is provider', async () => {
    const { sut } = makeSut()
    const httpRequest = makeFakeRequest(['email'])
  ...
  test('Should return 400 if password confirmation fails', async () => {
    const { sut } = makeSut()
    const httpRequest = makeFakeRequest(['passwordConfirmation'])
